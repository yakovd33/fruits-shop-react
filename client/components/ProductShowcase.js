...
const ProductShowcase = ({ id, name, price, salePrice, description, minAmount, badge, image, unit, cartItems, setCartItems, bottomAddToCart, type="product" }) => {
    ...
    return (
        <div className="product-showcase">
            ...
            { cartAmount == 0 &&
                <>
                    <div class={`mobile-add-to-cart-btn ${type != 'search' ? 'show-mobile' : ''}`} onClick={ (e) => handleAddToCart(e) }>
            ...
            { cartAmount > 0 &&
                <>
                    <div class={`cart-item-amount mobile-card-amount ${type != 'search' ? 'show-mobile' : ''}`}>
                        <div class="cart-item-amount-btn plus" onClick={ (e) => { productHandlePlus(id, cartItems, setCartItems, cartAmount, setCartAmount); handleAddToCart(e, true) } }>+</div>
                        <input type="text" class="cart-item-amount-number" readonly="" value={cartAmount}/>
                        <div class="cart-item-amount-btn minus" onClick={ (e) => productHandleMinus(id, cartItems, setCartItems, cartAmount, setCartAmount) }>
                            { cartAmount != 1 && '-' }
                    ...
                    { type != 'search' &&
                        <div class={`cart-item-amount desktop-card-amount hide-mobile`}>
                            <div class="cart-item-amount-btn plus" onClick={ (e) => { productHandlePlus(id, cartItems, setCartItems, cartAmount, setCartAmount); handleAddToCart(e, true) } }>+</div>
                            <input type="text" class="cart-item-amount-number" readonly="" value={cartAmount}/>
                            <div class="cart-item-amount-btn minus" onClick={ (e) => productHandleMinus(id, cartItems, setCartItems, cartAmount, setCartAmount) }>
                                { cartAmount != 1 && '-' }
